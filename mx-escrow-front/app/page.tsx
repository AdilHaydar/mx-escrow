...
export default function EscrowDashboard() {
  ...
  const fetchEscrows = async () => {
    try {
      ...
      const mockEscrows: Escrow[] = [
        ...
        { id: 2, seller: 'erd1...seller2', buyer: 'erd1...buyer2', amount: '200', status: 'Completed' },
  ...
  const updateEscrowStatus = async (escrowId: number, newStatus: 'Completed' | 'Cancelled') => {
    if (!isLoggedIn || !address) return
    try {
      setLoading(true)
      const functionName = newStatus === 'Completed' ? 'releaseEscrow' : 'cancelEscrow'
      const tx = {
        value: "0",
                      ...
                      <TableRow key={escrow.id} className="border-b border-gray-800">
                        ...
                        <TableCell>
                          <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                            ...
                            escrow.status === 'Completed' ? 'bg-blue-900 text-blue-300' :
                          ...
                          {escrow.status === 'Active' && (
                            <div className="space-x-2">
                              <Button
                                ...
                                onClick={() => updateEscrowStatus(escrow.id, 'Completed')}
                                disabled={loading}
                              >
                                Serbest Bırak
                              </Button>
